...
from propvg.apis import evaluate_model, set_random_seed
from propvg.datasets import build_dataset, build_dataloader
from propvg.models import build_model, ExponentialMovingAverage
from propvg.utils import (
    get_root_logger,
    load_checkpoint,
    init_dist,
    is_main,
...
def main_worker(cfg):
    ...
    if cfg.dataset == "GRefCOCO":
        ...
        cfg.dataset == "MixedSeg"
        or cfg.dataset == "RefCOCOUNC"
        or cfg.dataset == "RefCOCOPlusUNC"
        or cfg.dataset == "RefCOCOgUMD"
    ):
        excel_results = {
            "mIoU": [],
            "oIoU": [],
    ...
    for eval_loader, _prefix in zip(dataloaders, prefix):
        if is_main():
            logger = get_root_logger()
            logger.info(f"PropVG - evaluating set {_prefix}")
    ...
    if is_main():
        ...
        target_excel_path = os.path.join(
            work_dir,
            "refer_output_thr{}_{}_{}_{}_{}.xlsx".format(
...
def parse_args():
    parser = argparse.ArgumentParser(description="propvg-test")
    ...
    parser.add_argument("--MTD_K", default=250, type=int, help="K params in MTD")
...
def main():
    ...
    cfg.model.head["MTD"]["K"] = args.MTD_K
